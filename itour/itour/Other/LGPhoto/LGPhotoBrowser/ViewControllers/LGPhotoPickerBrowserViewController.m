...
@property (nonatomic, assign) BOOL needHidenToobar;
@end
...
- (void)setShowType:(LGShowImageType)showType
{
    ...
    if (self.showType != LGShowImageTypeImagePicker){
        ...
        self.needHidenToobar = YES;
    }
    if (self.showType == LGShowImageTypeImageBroswerAndPicker) {
        self.needHidenBar = NO;
        self.needHidenToobar = YES;
    }
}
...
- (void)selectedButtonTapped {
    ...
    if (selectAsset) {
        ...
        // 2. selectedAssets增加或删除
        if (_imagePickerSelectView.selectBtn.selected) {
            [self.selectedAssets addObject:self.photos[self.currentPage]];
        } else {
            [self.selectedAssets removeObject:self.photos[self.currentPage]];
        }
    }
...
- (void)setupXGToolbar
{
    if ((!_XGtoolbar)  && (!self.needHidenToobar)) {
        CGFloat height = 44;
...
- (void)updateSelectBtn {
    ...
    if (photoAsset) {
        ...
        LGPhotoPickerBrowserPhoto * photoPicker = self.photos[self.currentPage];
        for (LGPhotoPickerBrowserPhoto *asset in self.selectedAssets) {
            if ([[asset key] isEqual:[photoPicker key]]) {
                _imagePickerSelectView.selectBtn.selected = YES;
                break;
            }
        }
...
- (void)pickerPhotoScrollViewDidSingleClick:(LGPhotoPickerBrowserPhotoScrollView *)photoScrollView{
    if (self.needHidenBar && self.needHidenToobar) {
        [self dismissViewControllerAnimated:YES completion:nil];
        return;
    }
